...
/** Remove user-scoped residues:
 *  - UserSkill rows for skills the owner no longer uses anywhere
 *  - UserSkillMastery rows for skills the owner no longer uses anywhere
 */
async function cleanupUserSkillsTx(tx: Prisma.TransactionClient, userId: string): Promise<void> {
  ...
  if (stillUsed.size > 0) {
    const keep = Array.from(stillUsed);
    await tx.userSkill.deleteMany({ where: { userId, skillId: { notIn: keep } } });
    // ⬇ also drop mastery rows for skills no longer used
    await tx.userSkillMastery.deleteMany({ where: { userId, skillId: { notIn: keep } } });
  } else {
    // No sets/skills left for this owner → drop all user-scoped skill rows
    await tx.userSkill.deleteMany({ where: { userId } });
    await tx.userSkillMastery.deleteMany({ where: { userId } });
  }
}
    ...
    const set = await prisma.studySet.findUnique({
      ...
      include: {
        ...
        cards: {
          ...
          include: {
            // Relation from Card -> CardSkill[]  (your schema uses "skills")
    ...
    return NextResponse.json(
      {
        ...
        cards: set.cards.map((c) => ({
          ...
          position: c.position,
          imageUrl: c.imageUrl,
          inheritDefault: (c as any).inheritDefault ?? true,
...
export async function PATCH(req: NextRequest, { params }: { params: { id: string } }) {
  try {
    ...
    await prisma.$transaction(async (tx: Prisma.TransactionClient) => {
      ...
      // (4) GC user's personal skill & mastery library
