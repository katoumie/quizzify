  ...
  return (
    <div className="relative">
      ...
      <div
        ...
        className={`absolute right-0 mt-2 w-56 overflow-hidden rounded-lg border border-white/15 bg-[var(--bg)] shadow-lg transition z-[70] ${
          ...
          {(["public", "private", "friends"] as const).map((key) => (
            <button
              ...
              className={[
                ...
                value === key ? "bg-white/10 ring-1 ring-white/10" : "",
                "text-left",
              ].join(" ")}
            >
              ...
              {/* Right check same size as SkillCombo */}
              {value === key && <SvgFileIcon src="/icons/check.svg" className="h-5 w-5 shrink-0" />}
            </button>
          ))}
        </div>
      </div>
